// 
#define PROGNAME	"jcamsysxarchive"
#define VERSION         "0.1"
...
void update_messagemask(struct jcamsys_messagemask* mx, int cam)
{
	...
	for (i=1;i<JC_MAX_CAMS+1;i++)
	{
		...
		mx->camera_settings[i]=FALSE;							// server should send us any camera settings change
	}
	...
	mx->server_settings=FALSE;								// send MSG_SERVERSETTINGS when settings change
	mx->comment=FALSE;									// text comment from server processes.
}
...
int main( int argc, char *argv[ ] )
{
	...
	fd_archive		=	create_form_archive();
