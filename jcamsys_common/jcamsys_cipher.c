...
void random_md5(char *md)
{
	...
	bzero(&st,sizeof(st));
	jcam_generate_random_buffer((char*)&buf,sizeof(buf));
	str2md5((char*)&buf, sizeof(buf), (char*)&st, sizeof(st)); 
	strcpy(md,st);
}
...
int jcam_write_jcam_passwd(char *filename, char*pass, int passlen)
{
	...
	mdbuf[s]='\n';
	s++;
	r=savefile(filename,(char*)&mdbuf, s, 0644);
	if (r==s)
		return(JC_OK);
	return(JC_ERR);
}
...
int jcam_write_keyfile(char *filename, struct jcamsys_key* key)
{
	...
	{
		fprintf(file,"%02X",(unsigned char)key->xorkey[i]);
		l++;
		if (l==32)
		{
			l=0;
...
int jcam_locally_obfuscate_password(char *pw, int pwsize, char *obfuscated, int obsize, int len, int trueisenc)
{
	...
	idlen=loadfile("/etc/machine-id",(char*)&machineid,sizeof(machineid));
	if (idlen>32)
...
int jcam_getormake_client_obpassword(char* pw, int pwsize)
{
	...
	l=loadfile(".jcpass",(char*)&sbuf1,sizeof(sbuf1));								// Try current ?HOME directory first 
	if (l<=0)
		l=loadfile("/.jcpass",(char*)&sbuf1,sizeof(sbuf1));
	if (l<=0)
		l=loadfile("/root/.jcpass",(char*)&sbuf1,sizeof(sbuf1));
	if (l<=0)
		l=loadfile("/etc/jcamsys/.jcpass",(char*)&sbuf1,sizeof(sbuf1));
	if (l<=0)
	{
...
//printf("password len from getpass=%d\n",i);
		...
		{														// encode file
			...
			if (l>0)
			{
				l=savefile(".jcpass",(char*)&sbuf2,l,0644);					// write file
...
int jcam_read_compare_passphrase(struct jcamsys_key* key,int *fd, char*filename, int timeoutms)
{
	...
	r=loadfile(filename,(char*)&buf,sizeof(buf));
	if (r<0)
		return(JC_ERR);
...
// Pass around the shared key, for use on a trusted network
// contact the key exchange server (host,port), copy the shared key
// Returns key length on sucess or -1 on error
int jcam_key_exchange(char *kdata, int kdatasize, char *host, int portno)
{
	int ks=0;						// key size
	uint16_t kl=0;
        int sockfd;
	...
	sprintf(kdata,"?K");					// send request
	if (write(sockfd,kdata,strlen(kdata))<0)
	{
		close(sockfd);
	...
	kl=0;
	r=read(sockfd,&b,1);					// read key length
	kl=b;
	r=read(sockfd,&b,1);
	kl=kl+(b<<8);

	// Server should now send the key
	ks=0;
	do
	{
		r=read(sockfd,(char*)kdata+ks,1);
		if (r==1)
			ks++;
		if (r==0)
		{
...
//printf("r=%d ks=%d  \n",r,ks); fflush(stdout);
	} while ( (r!=0) & (ks<kl) );				// socket closed or complete
	close(sockfd);
	return(ks);						// return key length
}
