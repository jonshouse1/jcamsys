...
int run_command( char* cmd, char *text, int tsize)
{
	...
	if (fp == NULL) 
  	...
  	while (fgets(text, tsize-1, fp) != NULL) 
	{
    		//printf("%s", text);
  	}
...
// Return the percentage disk used for a given file or directory name
int volumepercent(char *filename)
{
	...
	sprintf(cmd,"df %s | awk '{ print $5 }' | tail -n 1",filename);
	if (run_command(cmd,(char*)&text,sizeof(text))==JC_OK)
	{
		p=atoi(text);
		return(p);
	}
...
int loadfile(char *filename, char *buf, int bufsize)
{
	int s=0;
	int fh=-1;
	int r=0;
...
int savefile(char *filename, char *buf, int bufsize, int mode)
{
	int fd=-1;
	int wb=0;
...
// Return pid of name partially matches a process name
// IE name="jcamsysserver"   finds "jcamsysserverx86_64"
#include <dirent.h>
