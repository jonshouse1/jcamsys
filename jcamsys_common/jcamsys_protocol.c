...
void print_timems(uint64_t tms)
{
	...
	printf("timestamp (ms)\t= %llu\t%s:",(long long unsigned int)tms,st);
	sprintf(st,"%llu",(long long unsigned int)tms);
	printf("%s\n",st+(strlen(st)-3));		// print last 3 digits only
}
...
void print_struct_jcamsys_image(struct jcamsys_image* jci)
{
	...
	strftime(st, sizeof(st), "%Y-%m-%d %H:%M:%S", info);
	printf("timestamp (ms)\t= %llu\t%s:",(long long unsigned int)jci->timestamp,st);
...
{
	printf("archive_files_written\t\t= %d\n",st->archive_files_written);
	printf("archive_file_write_errors\t= %d\n",st->archive_file_write_errors);

        // These values enable viewers to select window size and image dimensions
	printf("camera_largest_width_active\t= %d\n",st->camera_largest_width_active);
        printf("camera_largest_height_active\t= %d\n",st->camera_largest_height_active);
	printf("camera_largest_width_configured\t= %d\n",st->camera_largest_width_configured);
        printf("camera_largest_height_configured= %d\n",st->camera_largest_height_configured);
	fflush(stdout);
}
...
//printf("got valid header, need to read %d bytes\n",hdr.msglen); fflush(stdout);
		...
		else
		{
			printf("\n\n\njcam_read_message() -tried to read %d bytes in one chunk, got %d\n\n",
				(int)sizeof(struct jcamsys_header),r);
			fflush(stdout);
		}
	}
...
int jc_request_image(struct jcamsys_key* key, int sockfd, uint16_t reqid, int img, int cam)
{
	...
	if (jc_sockwrite(sockfd, (unsigned char*)&reqi, sizeof(struct jcamsys_req_image))<0)
		return(-1);
	return(rid);
}
...
int jc_request_timems(int sockfd, uint16_t reqid)
{
	...
	rid=jc_send_header(sockfd, JC_REQ_TIMEMS, sizeof(struct jcamsys_timems), reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)&sendbuf, 1)<0)
		return(-1);
	return(rid);
}
...
int jc_request_stats(int sockfd, uint16_t reqid)
{
	...
	rid=jc_send_header(sockfd, JC_REQ_STATS, sizeof(struct jcamsys_req_stats), reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)&sendbuf, sizeof(struct jcamsys_req_stats))<0)
		return(-1);
	return(rid);
}
...
{
	int rid=0;
	rid=jc_send_header(sockfd, JC_REQ_SERVERSETTINGS, 1, reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)&sendbuf, 1)<0)	// one misc byte
		return(-1);
	return(rid);
}
...
{
	int rid=0;
	struct jcamsys_sens_values*  msgv =(struct jcamsys_sens_values*)&sendbuf;
	int i=0;
	...
	rid=jc_send_header(sockfd, JC_MSG_SENS_VALUES, sizeof(struct jcamsys_sens_values),reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)&sendbuf, sizeof(struct jcamsys_sens_values))<0)
		return(-1);
	return(rid);
}
...
int jc_msg_sens_value(struct jcamsys_key* key, int sockfd, int16_t reqid, int senstype, float fvalue, int ivalue, char* cvalue)
{
	...
	sv.senstype = senstype;
	sv.sensor_fvalue   = fvalue;
	sv.sensor_ivalue   = ivalue;
	strcpy(sv.sensor_cvalue,cvalue);
	rid=jc_send_header(sockfd, JC_MSG_SENS_VALUE, sizeof(struct jcamsys_sens_value),reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)&sv, sizeof(struct jcamsys_sens_value))<0)
		return(-1);
	return(rid);
}
...
// For client to identify with server, also sent from server to client to set an ID
...
int jc_msg_camerasettings(struct jcamsys_key* key, struct jcamsys_camerasettings* cs, int sockfd, uint16_t reqid)
{
	...
	datetime(cs->dts,FALSE);		// always send datetime 
//sprintf(cs->dts,"hello world");
	rid=jc_send_header(sockfd, JC_MSG_CAMERASETTINGS, sizeof(struct jcamsys_camerasettings),reqid);
...
// Sends smallest packet with a payload, the send time is in the header
int jc_msg_timems(struct jcamsys_key* key, int sockfd, uint16_t reqid)
{
	int rid=0;

	//printf("jc_msg_timems()\n"); fflush(stdout);
	if (sockfd<0)
		return(-1);

	rid=jc_send_header(sockfd, JC_MSG_TIMEMS, sizeof(struct jcamsys_timems),reqid);
	// Send one bytes of whatever is in the buffer
	if (jc_sockwrite(sockfd, (unsigned char*)&sendbuf, 1)<0)
		return(-1);
	return(rid);
}
...
int jc_msg_stats(struct jcamsys_key* key, struct jcamsys_statistics* st, int sockfd, uint16_t reqid)
{
	...
	rid=jc_send_header(sockfd, JC_MSG_STATS, sizeof(struct jcamsys_statistics),reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)st, sizeof(struct jcamsys_statistics))<0)
		return(-1);
	return(rid);
}
...
int jc_msg_serversettings(struct jcamsys_key* key, struct  jcamsys_server_settings* s, int sockfd, uint16_t reqid)
{
	...
	rid=jc_send_header(sockfd, JC_MSG_SERVERSETTINGS, sizeof(struct jcamsys_server_settings),reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)s, sizeof(struct jcamsys_server_settings))<0)
		return(-1);
	return(rid);
}
 

// Generates a packet of type MSG_COMMENT with length len - comment should be zero terminated

int jc_msg_comment(struct jcamsys_key* key, int server, char* c, int sockfd, uint16_t reqid)
{
	int rid=0;
	struct jcamsys_comment	cm;

	bzero(&cm,sizeof(struct jcamsys_comment));
	cm.server=server;
	memcpy(&cm.comment,c,JC_COMMENT_LEN);				// take a local copy
	rid=jc_send_header(sockfd, JC_MSG_COMMENT, sizeof(struct jcamsys_comment) ,reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)&cm, sizeof(struct jcamsys_comment))<0)
		return(-1);
	return(rid);
}
...
{
	int rid=0;
	rid=jc_send_header(sockfd, JC_MSG_SERVERSTATE, sizeof(struct jcamsys_serverstate) ,reqid);
	if (jc_sockwrite(sockfd, (unsigned char*)ss, sizeof(struct jcamsys_serverstate))<0)
		return(-1);
	return(rid);
}
