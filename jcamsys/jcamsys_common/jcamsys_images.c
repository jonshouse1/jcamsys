...
void decodejpeg_error_exit (j_common_ptr cinfo)
{
        ...
        printf("decode_jpeg: error - "); fflush(stdout);
...
int decode_jpeg(unsigned char *jpg_buffer, unsigned long jpg_size, int *w, int *h, int *oc,unsigned char *bmp_buffer)
{
	...
	{
		printf("decode_jpeg() ends: %02X %02X exp:FF D9, not a jpeg\n",jpg_buffer[jpg_size-2],jpg_buffer[jpg_size-1]);
		fflush(stdout);
		return(JC_ERR);
	}
	if ( (jpg_buffer[0]!=0xFF) | (jpg_buffer[1]!=0xD8) )
	{
		printf("decode_jpeg() got:%02X %02X  exp:FF D8, not a jpeg\n",jpg_buffer[0],jpg_buffer[1]);
		fflush(stdout);
		return(JC_ERR);
	}
...
//JA new
	...
	{
		printf("Decoder has imahe %d x %d, ignoring it\n",cinfo.image_width,cinfo.image_height);
                fflush(stdout);
                jpeg_destroy_decompress (&cinfo);
		return(-2);
	}
